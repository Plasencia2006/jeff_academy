...
<div class="space-y-6" x-data="{ 
    ...
    <!-- Anuncios importantes/urgentes -->
    @php
        $anunciosImportantes = isset($anuncios) ? $anuncios->filter(function($a) {
            return $a->activo && 
                in_array($a->prioridad, ['importante', 'urgente']) && 
                (!$a->vigente_hasta || $a->vigente_hasta >= now()->toDateString());
        }) : collect();
    @endphp

    @if($anunciosImportantes->count() > 0)
    <div class="bg-gradient-to-r from-orange-50 to-red-50 border border-orange-200 rounded-xl p-4 sm:p-6">
        <div class="flex flex-col sm:flex-row sm:items-start space-y-4 sm:space-y-0 sm:space-x-4">
            ...
            <div class="flex-1 min-w-0">
                <h3 class="text-base sm:text-lg font-semibold text-orange-900 mb-2 text-center sm:text-left">
                    <i class="fas fa-star mr-2"></i>Anuncios Importantes
                </h3>
                <div class="space-y-2 sm:space-y-3">
                    @foreach($anunciosImportantes->take(3) as $anuncio)
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between space-y-3 sm:space-y-0">
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-2 mb-2">
                                        <h4 class="font-medium text-gray-900 text-sm sm:text-base break-words">
                                            {{ $anuncio->titulo }}
                                        </h4>
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium self-start
                                            @if($anuncio->prioridad === 'urgente')
                                                bg-red-100 text-red-800
                                            @else
                                                bg-orange-100 text-orange-800
                                            @endif">
                                            {{ ucfirst($anuncio->prioridad) }}
                                        </span>
                                    </div>
                                    <p class="text-xs sm:text-sm text-gray-700 mb-2 break-words">
                                        {{ $anuncio->mensaje }}
                                    </p>
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-1 sm:space-y-0 text-xs text-gray-500">
                                        ...
                                        <span class="flex items-center">
                                            ...
                                            <span class="truncate max-w-[150px]">
                                                {{ optional($anuncio->coach)->name ?? 'Administración' }}
                                            </span>
                                        </span>
                                        @if($anuncio->vigente_hasta)
                                            <span class="flex items-center">
                                                <i class="fas fa-clock mr-1"></i>
                                                Hasta {{ \Carbon\Carbon::parse($anuncio->vigente_hasta)->format('d/m/Y') }}
                                            </span>
                                        @endif
                                    </div>
                                </div>
                                @if($anuncio->enlace)
                                    ...
                                    class="w-full sm:w-auto sm:ml-4 inline-flex items-center justify-center px-3 py-2 text-orange-600 hover:text-orange-800 bg-orange-50 hover:bg-orange-100 rounded-md transition-colors text-sm">
