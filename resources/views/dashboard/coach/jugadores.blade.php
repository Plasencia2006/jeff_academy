                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($jugadoresData as $data)
                        ...
                        >
                            ...
                            <div class="mt-6 pt-4 border-t border-gray-200 flex justify-between space-x-2">
                                <button 
                                    @click="selectedPlayer = {{ $jugador->id }}; $dispatch('open-observaciones-modal')" 
                                    class="flex-1 px-3 py-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition text-sm font-medium"
                                >
                                    <i class="fas fa-eye mr-1"></i> Observaciones
                                </button>
                                <button 
                                    @click="selectedPlayer = {{ $jugador->id }}; $dispatch('open-estadisticas-modal')" 
                                    class="flex-1 px-3 py-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition text-sm font-medium"
                                >
                                    <i class="fas fa-chart-bar mr-1"></i> Estadísticas
                                </button>
                                <button 
                                    @click="selectedPlayer = {{ $jugador->id }}; $dispatch('open-asistencia-modal')" 
                                    class="flex-1 px-3 py-2 bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100 transition text-sm font-medium"
                                >
                                    <i class="fas fa-check mr-1"></i> Asistencia
                                </button>
                            </div>
                        </div>
                    @endforeach
